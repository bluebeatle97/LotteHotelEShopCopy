...
const Qna = () => {
    ...
    const [boardList, setBoardList] = useState([
        { no: "1", title: '언제 배송이 되나요?', description: '어제부터 기다렸는데 아직 배송이 안됐어요.', viewCount: 1 },
        { no: "2", title: '31일에 주문했는데 언제 배송 시작하나요?', description: '31일에 주문했습니다.', viewCount: 2 },
    ]);
    // UI 상태들
    ...
    const boardSave = () => {
        ...
        // 현재 게시판에 글이 하나라도 있으면 가장 큰 게시글 번호를 찾고 게시글이 하나도 없으면 0으로 설정
        const maxNo = boardList.length > 0 ? Math.max(...boardList.map(b => Number(b.no))) : 0;
        // 새 글 추가
        const newBoard= {
            no: (maxNo + 1).toString(),  // 가장 큰 번호 + 1 → 새 게시글 번호
            title: title,                 // 입력한 제목
            description: description,  // 입력한 내용
    ...
    return (
        <div>
            ...
            {writeOk && (
                <div style={{ marginTop: "30px" }}>
                    <h5  style={{textAlign: "left" }}>문의글 남기기</h5>
                    {/* 오류 메시지 표시 */}
                    {errorMessage && (
